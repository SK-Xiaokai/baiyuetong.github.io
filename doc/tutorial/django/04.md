...
我们后端开发大都基本都需要操作数据，包括数据的 存储、查询、修改、删除。

通常，这些都是通过数据库来完成的。 

Django 把对数据的操作，用一个 **Model**  类型的对象封装起来。

通常，一个 Model 对象就 对应了数据库中的一张表， 也包括了对该表中数据的处理方法 。

在Django中

- 定义一张数据库表 就是定义一个继承自 django.db.models.Model 的类

- 定义该表中的列， 就是定义该类里面的一些属性。
  
这样，开发者对数据库的访问，从原来的使用底层的 sql 语句，变成 面向对象的开发，通过一系列对象的类定义 和方法调用就可以 操作数据库。   就是说无需使用sql语句操作数据库了。

这样，

首先 极大的简化了我们应用中的数据库开发， 提高了开发的效率；
...
关系型数据库，目前最为常用的是 mysql 和 postgresql。都是开源数据库。

建议大家实际工作中使用的时候，使用上面这两种。

但是为了省掉搭建数据库服务 和安装客户端库 的麻烦， 我们的教程里面，使用另一种更简单的 数据库 sqlite

sqlite 没有 独立的数据库服务进程，数据操作被做成库直接供应用程序调用。 Django中可以直接使用，无须先搭建数据服务。

本教程就用sqlite数据库，如果大家要使用mysql 等其他数据库 只需修改一些配置就可以了。
...
![default](https://user-images.githubusercontent.com/36257654/38779059-afbf0336-40f5-11e8-8329-06dc772528c8.png)


我们可以点击下面的链接，下载sqlite 数据库工具 sqlitestudio

https://sqlitestudio.pl/index.rvt?act=download

![default](https://user-images.githubusercontent.com/36257654/38779103-4470ad40-40f6-11e8-9b25-747014e9bbed.png)

下载后解压即可， 运行该工具， 然后打开上面的 文件 ```db.sqlite3```

可以发现该库中有下面这些表已经创建好了。

![default](https://user-images.githubusercontent.com/36257654/38766663-168a9c00-4007-11e8-9843-d7566c7c5a2a.png)
...
执行一下，会发现在 common\migrations 目录下面出现了0001_initial.py
...
![default](https://user-images.githubusercontent.com/36257654/38766716-934bde6a-4008-11e8-8262-42b288c6eff7.png)
...
![default](https://user-images.githubusercontent.com/36257654/38766977-bce4eb82-400c-11e8-897d-6ebcf4958cd3.png)


<br>
点击下面的 ADD CUSTOMER 按钮来添加一条客户记录

![default](https://user-images.githubusercontent.com/36257654/38766980-d2bcd7bc-400c-11e8-9adf-ee11262085de.png)

<br><br>
在跳出的界面中输入客户信息后，点击SAVE按钮

![default](https://user-images.githubusercontent.com/36257654/38767020-64fa25c6-400d-11e8-9a2e-08347a091dc6.png)
...
![default](https://user-images.githubusercontent.com/36257654/38767042-adf03e8c-400d-11e8-98bc-5d554bbc50c4.png)

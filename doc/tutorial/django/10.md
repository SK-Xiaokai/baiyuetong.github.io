...
但是，我们的情况要更复杂一些， 因为 一个订单对应的药品 不仅有多个， 而且订单中每个药品都要有数量字段。

该怎么办呢？

我们这时，就需要自己定义 一张表示多对多关系的表，像下面这样
...
我们 删除掉amount字段， 修改 medicine字段，添加一个参数  ```through``` ，用例指明 order 和 medicine的多对多关系表 是   ```OrderMedicine``` 
...
用 查看工具查看数据库，发现创建了一张名为 common_medicine  和  common_order 的表。

另外，Django 还会 产生的一张 表 （这里就是 common_ordermedicine）来 实现 order 表 和 medicine 表之间的多对多的关系。

...
修改后，再次访问 http://127.0.0.1/agent/customers/

得到如下内容

![default](https://user-images.githubusercontent.com/36257654/38863071-2dbb28fe-4269-11e8-847e-d2abc8d8af76.png)
...
这样 服务端就**只负责提供数据**， 界面的构成全部在前端进行，称之为前端渲染。

前端渲染的基本原理其实和后端渲染类似， 也是程序代码内嵌入html页面中， 调用模板引擎去 动态生成html。 

只是这个工作在前端（浏览器端）执行， 使用前端的 框架库去完成，比如angular，react，vue。
